    ...
    for (int i = 0; i < resources.length; i++) {
      ...
      if (resource instanceof ClassPathResource) {
        ...
        throw new IllegalArgumentException(
            "Unsupported resource type for file source: " + resource.getClass());
      }
    }
    return sources;
  }
  ...
  public BinaryFile getBinaryFileNamed(String name) {
    for (FileSource resource : this.sources) {
      ...
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "Caught exception while trying to create file handle for file "
                  + name
                  + ", trying next FileSource",
              e);
        }
      } catch (IOException e) {
        if (log.isDebugEnabled()) {
  ...
  public FileSource child(String subDirectoryName) {
    ...
    for (FileSource resource : this.sources) {
      ...
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "Caught exception while trying to create file source for "
                  + subDirectoryName
  ...
  public String getPath() {
    for (FileSource resource : this.sources) {
      ...
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "Caught exception while trying to create URL file handler for "
                  + resource.getPath()
                  + ", continuing with next source",
              e);
        }
