...
class AcuantWireMockApplicationTest {
  ...
  @Autowired private AcuantImageUploadUtil imageUploadUtil;
  ...
  void testExpiredResponse() throws Exception {
    ...
    AcuantResponse response =
        imageUploadUtil.uploadAndGetResponse("http://localhost:" + serverPort, ognlExpression);
  ...
  void testUnknownDocTypeResponse() throws Exception {
    String ognlExpression =
        "#action = #this.alerts.{? #this.key==\"Document Classification\"}[0], #this.alerts.clear, #action.result=2, #this.alerts.add(#action)";
    AcuantResponse response =
        imageUploadUtil.uploadAndGetResponse("http://localhost:" + serverPort, ognlExpression);
